...
bool IonizationAmplitude::run (
	...
	for (size_t idir = 0; idir < dirs.size(); idir++)
	{
		...
		double Eshare = dirs[idir].second.z / dirs[idir].first.z;
		
		// compute outgoing momenta so that
		//   1) (k₁)² + (k₂)² = Ei
		//   2) (k₂)² / (k₁)² = Eshare
		rArray k1 = sqrt(E_arr / (1 + Eshare));
		rArray k2 = sqrt(E_arr / (1 + Eshare) * Eshare);
		...
		for (size_t ie = 0; ie < E_arr.size(); ie++)
		{
			...
			for (size_t il = 0; il < Lll_arr[ie].size(); il++)
			{
				...
				Complex f = cheb_arr[ie][il].clenshaw(k1[ie],cheb_arr[il][il].tail(1e-8)) / gsl_sf_pow_int(k1[ie] * k2[ie], 2);
