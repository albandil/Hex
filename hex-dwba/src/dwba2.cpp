...
//     exit(0);
    ...
    auto integrand = [&](double r1) -> Complex {
        ...
        {
            // infinite radius corresponds to infinity, which is damped by "phi"
            if (not finite(r2))
                return 0.;
            
            // if r2 >= r0 (classical turning point), multiply worrilessly
            if (r2 >= min(geta.getTurningPoint(), phii.getTurningPoint(), chii.getTurningPoint()))
                return geta(r2) * phii(r2) * chii(r2);
            
            // otherwise (r2 < r0) combine asymptotic behaviour of the three functions
            Complex ygeta, yphii, ychii;
            int kgeta, kphii, kchii;
            std::tie(ygeta,kgeta) = geta.getZeroAsymptotic(r2);
            std::tie(yphii,kphii) = phii.getZeroAsymptotic(r2);
            std::tie(ychii,kchii) = chii.getZeroAsymptotic(r2);
            return std::pow(r2, kgeta + kphii + kchii) * ygeta * yphii * ychii;
        };
