...
{
    /**
     * @brief Evaluate hydrogenic bound state.
     * 
     * By the "bound state" it is meant here the spherical function
     * \f$ P_{nl}(r) = rR_{nl}(r) \f$.
     */
...
public:
    ...
    /// Determine whether the object represents a bound state.
    inline bool isBound() const { return n != 0; }
    
    /// Determine whether the object represents a free state.
    inline bool isFree() const { return n == 0; }
    ...
    /// Classical turning point (only for free states).
...
/**
 * Hydrogen radial function - sturmian.
 */
...
public:
    ...
    /// Get shielding constant (= range parameter).
...
private:
    ...
    /// Shielding factor (range parameter).
    double lambda;
};
