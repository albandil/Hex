...
//         double phi = iphi * special::constant::pi / (Nphi - 1);
        ...
        for (int Ln = 0; Ln < Nn and Ln <= maxLn; Ln++)
        {
            ...
            auto integrand_Ub_wrap = [ki,kf,vki,vkf,Nn,Ln,Qmax,Etot](int n, double const * coords) -> Complex
            {
                // check dimensions
                assert(n == 3);
            ...
            std::cout << underline(format("Sparse grid integration of bound U for θ = %g, φ = %g", theta, phi)) << std::endl;
