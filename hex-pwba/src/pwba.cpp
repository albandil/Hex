...
double compute_Iexc(int li, int lf, int lam, int Ni, int Li, double ki, int Nf, int Lf, double kf)
{
	if (lam == 0)
	{
// 		printf("Computing Iexc (li=%d,lf=%d,lam=%d,Ni=%d,Li=%d,ki=%g,Nf=%d,Lf=%d,kf=%g)... ",
// 			   li, lf, lam, Ni, Li, ki, Nf, Lf, kf);
// 		fflush(stdout);
		...
		auto outer_integrand = [ = ](double x) -> double {
			...
			auto inner_integrand = [ = ](double y) -> double {
				try {
					return (1./y - 1./x) * ric_j(li,ki*y) * hydro_P(Nf,Lf,y);
				} catch (...) {
					return 0.;
				}
			};
...
int main(int argc, char* argv[])
{
	if (argc != 8)
	{
		printf("\nUsage:\n\tmborn <ni> <li> <nf> <lf> <maxL> <Ei> <sigmaeps>\n\n");
		exit(0);
	}
...
// 				Iexc[li_lf_lam] = compute_Iexc(li, lf, lam, Ni, Li, ki, Nf, Lf, kf);
	...
	if (maxL >= maxl_limit)
		fprintf(stderr,
				"Warning: Partial wave limit %d reached for E = %g Ry, the result may lack the desired presicion.\n",
				maxl_limit, Ei
		);
	...
	for (int Mf = -Lf; Mf <= Lf; Mf++)
	{
		...
		printf("%g\t%g\t%g\t", tcs.first + tcs.second, tcs.first, tcs.second);
	}
	printf("%d\n", maxL);
