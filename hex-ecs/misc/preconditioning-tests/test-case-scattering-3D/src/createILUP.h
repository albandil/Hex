    ...
    std::vector<std::vector<int>> adjc (N*N*N), adjl (N*N*N);
    ...
    NumberArray<LU_int_t> P (N*N*N + 1);
    NumberArray<LU_int_t> I ((nnz + N*N*N)/2);
    NumberArray<Complex>  X ((nnz + N*N*N)/2);
    for (int i = 0, n = 0; i < N*N*N; i++)
    {
        int idx = A.p()[i];
    ...
    // create the L matrix
    CsrMatrix<LU_int_t,Complex> ilupU (std::size_t(N*N*N), std::size_t(N*N*N), std::move(P), std::move(I), std::move(X));
